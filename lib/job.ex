defmodule Job do
  ...
  def set_timezone(%Job{opts: opts} = job, timezone), do: %{job | opts: Keyword.put(opts, :timezone, timezone)}
  def set_overlap(%Job{opts: opts} = job, overlap), do: %{job | opts: Keyword.put(opts, :overlap, overlap)}
  def set_context(%Job{} = job, context), do: %{job | context: context}

  def dummy() do
    Job.new()
    |> Job.set_name(:test) |> Job.set_task(fn -> :ok end)
    |> Job.set_schedule({1000, :ms}) |> Job.set_timezone(:utc) |> Job.set_overlap(true)
  ...
  def dummy2() do
    ...
    Job.new()
    |> Job.set_name(:test) |> Job.set_task(fn -> :ok end)
    |> Job.set_schedule(~e[* * * * * * *]e) |> Job.set_timezone(:utc) |> Job.set_overlap(true)
    |> Job.set_context(%{strip_name: :john, job: :repeater})
  end
end
